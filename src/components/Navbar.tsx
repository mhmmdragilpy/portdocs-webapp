...
export default async function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border-b border-slate-200 dark:border-slate-700/50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-2 group">
              <div className="p-2 bg-blue-100 dark:bg-blue-500/20 rounded-xl group-hover:bg-blue-200 dark:group-hover:bg-blue-500/30 transition-colors">
                <Ship className="w-6 h-6 text-blue-600 dark:text-blue-400" />
              </div>
              <span className="font-bold text-xl tracking-tight text-slate-900 dark:text-white">
                Port<span className="text-blue-600 dark:text-blue-400">Docs</span>
              </span>
            </Link>
          </div>
            ...
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <form action="/api/auth/signout" method="POST">
                  <button type="submit" className="text-slate-600 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 p-2 transition-colors">
              ...
              <form action="/api/auth/google" method="POST">
                <button type="submit" className="flex items-center gap-2 bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-md">
